...
<div class="terminal-output">
  {#each output.entries as entry (entry.metadata.sequence)}
    <span class:error={entry.operation === "stderr"} class:output={entry.operation === "stdout"}>
      {String(entry.data)}
    </span>
  {/each}
</div>
...
The `entries` property provides a read-only array of visible stdout and stderr entries. Calling `terminal.clearOutput()` empties the array and updates the markup. Listen to the terminal output events when you need a transcript that includes clear operations and does not empty.
